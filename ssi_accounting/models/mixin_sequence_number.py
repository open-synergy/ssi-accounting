...
class MixinSequenceNumber(models.AbstractModel):
    """Date-aware gapless sequence numbering mixin for statutory documents.

    Ported verbatim (behaviour-wise) from Odoo core
    ``addons/account/models/sequence_mixin.py`` (class ``SequenceMixin``,
    model ``sequence.mixin``), because that mixin lives inside the
    ``account`` module, which ``ssi_accounting`` must not depend on.

    Why this mixin instead of ``mixin.sequence`` (``ssi_sequence_mixin``):
    ``mixin.sequence`` numbers a document from a ``sequence.template`` +
    ``ir.sequence`` configuration record and has no notion of the
    document's own accounting date. It cannot express any of the six
    capabilities a statutory journal entry number needs, all of which this
    mixin provides instead:

    * Deriving the sequence format directly from the document's own
      accounting date field (``_sequence_date_field``), instead of an
      externally configured template.
    * Resetting the running number when the year (or month) implied by
      the date changes (``_deduce_sequence_number_reset``).
    * Discovering the prefix/format to continue from the last matching
      document, rather than from a fixed ``ir.sequence`` counter
      (``_get_last_sequence``).
    * Rejecting a document whose existing number does not match its own
      accounting date (``_constrains_date_sequence``).
    * Detecting gaps in an existing numbering chain
      (``_is_last_from_seq_chain`` / ``_is_end_of_seq_chain``).
    * Concurrency-safe increments guarded by a unique DB index on the
      sequence field, rather than by locking an ``ir.sequence`` row
      (``init``, ``_locked_increment``).

    ``_name`` is deliberately ``mixin.sequence_number`` (not
    ``mixin.sequence``) to avoid any XML ID / inheritance collision with
    ``ssi_sequence_mixin``'s ``mixin.sequence``: a concrete model such as
    ``journal_entry`` must never inherit both.

    This module only provides the mixin. Wiring it into a concrete model
    (numbering unit + state machine) is deliberately out of scope for this
    port; see ``models/test_mixin_sequence_number.py`` for the minimal
    concrete fixture used to exercise it in this module's own test suite.
    """

    _name = "mixin.sequence_number"
    _description = "Automatic Sequence Number"
    ...
    _sequence_year_range_monthly_regex = (
        rf"^{prefix}{year}{prefix2}{year_end}(?P<prefix3>\D){month}"
        rf"(?P<prefix4>\D+?){seq}{suffix}$"
    )
    ...
    _sequence_yearly_regex = (
        rf"^{prefix}(?P<year>((?<=\D)|(?<=^))((19|20|21)?\d{{2}}))"
        rf"(?P<prefix2>\D+?){seq}{suffix}$"
    )
    ...
    sequence_prefix = fields.Char(
        ...
        store=True,
        help="Static portion of the document number, ahead of the "
        "running sequence digits (e.g. the 'TEST/2026/' part of "
        "'TEST/2026/00001').",
    )
    sequence_number = fields.Integer(
        compute="_compute_split_sequence",
        store=True,
        help="Running numeric portion of the document number, parsed "
        "back out of the number itself (e.g. 1 for 'TEST/2026/00001').",
    )
    ...
    def init(self):
        ...
        if not self._abstract and self._sequence_index:
            ...
            if not index_exists(self.env.cr, index_name):
                self.env.cr.execute(
                    ...
                    CREATE INDEX %(index_name)s ON %(table)s
                        (%(sequence_index)s, sequence_prefix desc,
                         sequence_number desc, %(field)s);
            ...
            if not unique_index:
                _logger.warning(
                    "A unique index for `mixin.sequence_number` is missing "
                    "on %s. This will cause duplicated sequences under "
                    "heavy load.",
                    self._table,
                )
    ...
    def _get_sequence_cache(self):
        ...
        # sequence numbers within a single transaction, we cache the
        # sequence value for the duration of the in-flight transaction.
        #
        # We use `cr.cache` and expects any savepoint rollback or commit to
        # clear the cache. This is important because as soon as the lock is
        # released, there might be a record consumming the next sequence in
        # a concurent transaction for which the cache can't be aware.
        #
        # Before adding an entry for a sequence to this
        # `mixin.sequence_number` cache, the transaction must have locked
        # the corresponding unique constraint, typically by successfully
        # updating or inserting a row governed by the constraint (note: be
        # mindful of partial constraint clauses).
        #
        # Entries in the mixin.sequence_number cache will look like this:
        # {
    ...
    def _sequence_matches_date(self):
        ...
        year_match = not format_values["year"] or self._year_match(
            format_values["year"], forced_year_start or date_start.year
        )
        year_match = year_match and (
    ...
    def _constrains_date_sequence(self):
        # Make it possible to bypass the constraint to allow edition of
        # already messed up documents.
        # /!\ Do not use this to completely disable the constraint as it
        # will make this mixin unreliable.
...
""",
    ...
    def _deduce_sequence_number_reset(self, name):
        ...
        :param name: the sequence that is used as a reference to detect
            the resetting periodicity. Typically, it is the last before
            the one you want to give a sequence.
        """
        for regex, ret_val, requirements in [
            ...
            if match:
                ...
                if (
                    ...
                    # year and year_end are not compatible for range (the
                    # difference is not 1)
                    continue
    ...
    def _get_last_sequence_domain(self, relaxed=False):
        ...
        This function should be overriden by models inheriting from this
        mixin.
        ...
        :returns: tuple(where_string, where_params): with
            ...
            where_params: a dictionary containing the parameters to
                substitute at the execution of the query.
        """
        self.ensure_one()
        return "", {}
    ...
    def _get_starting_sequence(self):
        ...
        This function should be overriden by models heriting from this
        mixin. This number will be incremented so you probably want to
    ...
    def _get_last_sequence(self, relaxed=False, with_prefix=None):
        ...
        This is done by taking the number with the greatest alphabetical
        value within the domain of _get_last_sequence_domain. This means
        that the prefix has a huge importance.
        For instance, if you have INV/2019/0001 and INV/2019/0002, when
        you rename the last one to FACT/2019/0001, one might expect the
        next number to be FACT/2019/0002 but it will be INV/2019/0002
        (again) because INV > FACT. Therefore, changing the prefix might
        not be convenient during a period, and would only work when the
        numbering makes a new start (domain returns by
        _get_last_sequence_domain is [], i.e: a new year).

        :param relaxed: this should be set to True when a previous
            request didn't find something without. This allows to find a
            pattern from a previous period, and try to adapt it for the
            new period.
        :param with_prefix: The sequence prefix to restrict the search
            on, if any.

        :return: the string of the previous sequence or None if there
            wasn't any.
        """
        self.ensure_one()
        if (
        ...
        query = f"""
                ...
                {where_string}
                AND sequence_prefix = (SELECT sequence_prefix
                    FROM {self._table} {where_string}
    ...
    def _get_sequence_format_param(self, previous):
        ...
        :returns: a 2-elements tuple with:
            ...
            - format is the format string on which we should call
              .format()
            - format_values is the dict of values to format the `format`
              string ``format.format(**format_values)`` should be equal
              to ``previous``
        """
        ...
        if (
            ...
            # if we don't have a seq, consider we only have a prefix and
            # not a suffix
    ...
    def _locked_increment(self, format_string, format_values):
        """Increment the sequence for the given format, returning the new
        value.

        This method will lock the sequence in the database through its
        unique constraint, in order to ensure cross-transactional
        uniqueness of sequence numbers. If the sequence is already locked
        by another transaction, it will wait until the other one
        finishes, then grab the next available number.

        Once the sequence has been locked by the transaction, further
        increments will rely on a cache, to avoid the need for multiple
        savepoints (see implementation comments)

        At entry, the sequence record must be governed by the unique
        constraint, e.g. for an account.move, it must be in state
        `posted`, otherwise the lock won't be taken, and sequence numbers
        may not be unique when returned.
        """
        cache = self._get_sequence_cache()
        seq = format_values.pop("seq")
        # cache key unique to a sequence: its format string + its
        # sequence index
        cache_key = (
        ...
        with self.env.cr.savepoint(flush=False) as sp:
            # By updating a row covered by the sequence's UNIQUE
            # constraint, the transaction acquires an exclusive lock on
            # the corresponding B-tree index entry. This prevents other
            # transactions from inserting the same sequence value. See
            # _bt_doinsert() and _bt_check_unique() in the PostgreSQL
            # source code.
            #
            # This guarantee holds only if the sequence row is currently
            # covered by a unique index, so any partial index conditions
            # must be satisfied beforehand.
            #
            # This operation requires a savepoint because, after waiting
            # for the lock, the transaction may discover that the new
            # number is already taken, resulting in a constraint
            # violation. Such violations cannot be cleanly recovered from
            # without a savepoint. In that case, we retry until a free
            # number is found.
            #
            # Unfortunately, repeated savepoints can severely impact
            # performance, so we minimize their use. Once the lock is
            # acquired, we rely on a transactional cache provided by
            # _get_sequence_cache.
            # Because the transaction holds the lock on the initially
            # assigned sequence number, other transactions must wait for
            # its completion before assigning newer numbers. It is
            # therefore safe to continue assigning sequential numbers
            # without additional savepoints.
            #
            # See also:
            #  - https://postgres.ai/blog/20210831-postgresql-subtransactions-considered-harmful
            ...
            while True:
                ...
                try:
                    self.env.cr.execute(
                        SQL(
                            "UPDATE %(table)s SET %(fname)s = %(sequence)s "
                            "WHERE id = %(id)s",
                            table=SQL.identifier(self._table),
    ...
    def _set_next_sequence(self):
        ...
        This method ensures that the field is set both in the ORM and in
        the database. This is necessary because we use a database query
        to get the previous sequence, and we need that query to always
        be executed on the latest data.
        """
        self.ensure_one()
    ...
    def _get_next_sequence_format(self):
        ...
        This method retrieves the last used sequence and determines the
        next sequence format based on it. If there is no previous
        sequence, it initializes a new sequence using the starting
        sequence format.

        :returns: a 2-element tuple with:

            - format_string (str): the string on which we should call
              .format()
            - format_values (dict): the dict of values to format
              ``format_string``
        """
    ...
    def _is_last_from_seq_chain(self):
        """Tells whether or not this element is the last one of the
        sequence chain.
    ...
    def _is_end_of_seq_chain(self):
        """Tells whether or not these elements are the last ones of the
        sequence chain.
