...
* ``debit``/``credit``/``balance``/``amount_currency`` stay consistent
  whichever of the three is filled first: ``balance`` is a compute+inverse
  pair over ``debit``/``credit``, and ``amount_currency`` is a compute+inverse
  pair over ``balance``/``currency_rate`` -- editing any one of the three
  keeps the others in sync, exactly like upstream ``account.move.line``.
